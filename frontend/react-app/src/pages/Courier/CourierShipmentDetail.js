// pages/Courier/CourierShipmentDetail.jsx
...
// Defines which statuses a courier may transition to from each current status.
// Normal delivery flow and return flow are kept separate.
const ALLOWED_NEXT = {
...
// Statuses that require the courier to capture a delivery signature
...
export default function CourierShipmentDetail() {
  ...
  // ── Fetch shipment + status types ───────────────────────────
  useEffect(() => {
    if (!token || !id) return;
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/courier/shipments/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      }),
      fetch(`${API}/api/courier/status-types`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then((r) => r.json()),
    ])
  ...
  // ── Compute which status options to show ────────────────────
  const allowedStatusTypes = useMemo(() => {
    ...
    const allowed     = ALLOWED_NEXT[currentName] ?? [];
    return statusTypes.filter((t) => allowed.includes(t.name));
  }, [statusTypes, shipment]);

  // If the selected status got removed from the allowed list after a refresh, clear it
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    const needsSig     = SIGNATURE_STATUSES.includes(selectedType?.name);
    ...
    try {
      const r = await fetch(`${API}/api/courier/shipments/${id}/status`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ StatusTypeId: selectedStatus, SignatureDataUrl: signatureDataUrl }),
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data || `HTTP ${r.status}`);

      setSaveResult({ ok: true, msg: `Būsena atnaujinta: ${data.newStatusName}` });

      const refreshed = await fetch(`${API}/api/courier/shipments/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then((res) => res.json());
    ...
    return (
      <div className="cr-page cr-page--detail">
        <div className="cr-empty cr-empty--error">
          ...
          <span>{error ?? "Siunta nerasta"}</span>
        </div>
      </div>
    );
      ...
      <div className="cr-detail-grid">
        ...
        <div className="cr-section">
          <div className="cr-section-header">
            <FiUser size={14} />
            <span className="cr-section-title">Klientas</span>
          </div>
        ...
        <div className="cr-section">
          <div className="cr-section-header">
            <FiMapPin size={14} />
            <span className="cr-section-title">Pristatymo adresas</span>
          </div>
          <div className="cr-info-list">
        ...
        <div className="cr-section cr-section--wide">
          <div className="cr-section-header">
            <FiTruck size={14} />
            <span className="cr-section-title">Siuntos informacija</span>
          </div>
          <div className="cr-info-list">
            <div className="cr-info-row">
            ...
            {shipment.courierName && (
              <div className="cr-info-row">
                ...
                <span className="cr-info-value">
                  {shipment.courierName}{shipment.courierType ? ` (${shipment.courierType})` : ""}
                </span>
              </div>
            )}
            <div className="cr-info-row">
              ...
              {shipment.packages.map((pkg, i) => (
                <div key={pkg.id_Package} className="cr-package-row">
                  ...
                  <div className="cr-package-info">
                    ...
                    <div className="cr-package-tracking" style={!pkg.trackingNumber ? { opacity: 0.4 } : {}}>
        ...
        <div className="cr-section cr-section--wide">
          <div className="cr-section-header">
            <FiClock size={14} />
            <span className="cr-section-title">Būsenų istorija</span>
          </div>
        ...
        {isDone ? (
          ...
          <div className="cr-section cr-section--wide">
            <div className="cr-alert cr-alert--error">
              ...
              Šiai būsenai nėra galimų perėjimų.
            </div>
          </div>
        ) : (
          <div className="cr-section cr-section--wide cr-section--action">
            <div className="cr-section-header">
              <FiEdit3 size={14} />
              <span className="cr-section-title">Atnaujinti būseną</span>
            </div>
            ...
            {needsSignature && (
              <div className="cr-sig-wrap">
                ...
                <div className="cr-sig-canvas-wrap">
                  ...
                  />
                  {!hasSig && (
                    <div className="cr-sig-placeholder">Pasirašykite čia</div>
                  )}
                </div>
                <div className="cr-sig-hint">
                  <FiAlertCircle size={13} />
                  Parašas privalomas pristatymui patvirtinti
                </div>
              </div>
            )}
